...
! landunit number for urban, lake, wetland, and glacier patches
!
#if (defined DGVM)
  integer, parameter :: maxpatch_pft = 10
#else
  integer, parameter :: maxpatch_pft = 4                 !max number of PFT subgrid patches per grid cell
#endif
  integer, parameter :: npatch_urban = maxpatch_pft + 1  !urban   
  integer, parameter :: npatch_lake = npatch_urban + 1   !lake    
  integer, parameter :: npatch_wet = npatch_lake  + 1    !wetland 
  integer, parameter :: npatch_glacier= npatch_wet + 1   !glacier 
  integer, parameter :: maxpatch = npatch_glacier        !max number of subgrid gridcell patches 
!
