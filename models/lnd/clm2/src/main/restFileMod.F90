...
#if (defined DGVM)
    use restDGVMMod, only : restart_dgvm
#endif
#if (defined COUP_CSM)
...
!-----------------------------------------------------------------------
   ...
   ! pft energy state variable - t_ref2m_min
    ...
    do pi = begp,endp
       p => ppoint(pi)%p
       if (flag == 'read' ) p%pes%t_ref2m_min = rbuf1dp(pi)
       if (flag == 'write') rbuf1dp(pi) = p%pes%t_ref2m_min
    end do
   ...
   ! pft energy state variable - t_ref2m_max
    ...
    do pi = begp,endp
       p => ppoint(pi)%p
       if (flag == 'read' ) p%pes%t_ref2m_max = rbuf1dp(pi)
       if (flag == 'write') rbuf1dp(pi) = p%pes%t_ref2m_max
    end do
    if (flag == 'write') call wrtout (nio, rbuf1dp, clmlevel=pfts1d%name)  
   ...
   ! pft energy state variable - t_ref2m_min_inst
    ...
    do pi = begp,endp
       p => ppoint(pi)%p
       if (flag == 'read' ) p%pes%t_ref2m_min_inst = rbuf1dp(pi)
       if (flag == 'write') rbuf1dp(pi) = p%pes%t_ref2m_min_inst
    end do
    if (flag == 'write') call wrtout (nio, rbuf1dp, clmlevel=pfts1d%name)  
   ...
   ! pft energy state variable - t_ref2m_max_inst
    ...
    do pi = begp,endp
       p => ppoint(pi)%p
       if (flag == 'read' ) p%pes%t_ref2m_max_inst = rbuf1dp(pi)
       if (flag == 'write') rbuf1dp(pi) = p%pes%t_ref2m_max_inst
    end do
    if (flag == 'write') call wrtout (nio, rbuf1dp, clmlevel=pfts1d%name)  
...
!-----------------------------------------------------------------------
   ...
   if (masterproc) then
      if (flag == 'write') then
         ...
         if (mss_irt > 0) then
!            rem_dir = trim(archive_dir) // '/rest/' MSP overrode this.
            rem_dir = trim(archive_dir)
...
!       rem_dir = trim(archive_dir) // '/rest/' MSP override
       rem_dir = trim(archive_dir)
