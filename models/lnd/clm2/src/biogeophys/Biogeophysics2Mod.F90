...
!
    ...
    real(r8), pointer :: t_rad_col       !column-level radiative temperature (Kelvin)
...
!-----------------------------------------------------------------------
    ...
    do pi=1,cps%npfts
       ...
       if (qflx_evap_soi > 0._r8 .and. topsoil_evap_tot > 0._r8) then
    ...
    pftsum = 0._r8
    do pi=1,cps%npfts
       pftsum = pftsum + c%p(pi)%pef%eflx_lwrad_out * c%pw(pi)   
    end do
    t_rad_col = (pftsum/sb)**0.25
