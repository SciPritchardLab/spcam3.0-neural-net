...
initFile = sys.argv[2]
# making nickname for job (unclear as to whether or not there are restrictions on job name. Will ask later.
nickname = re.sub("^[^-]+", "", initFile)
nickname = re.sub("[^-]+$", "", nickname)
nickname = re.sub("-", "", nickname)
nickname = str(modelNum) + nickname
...
#Copying in initial condition file + print path/name as a sanity check
