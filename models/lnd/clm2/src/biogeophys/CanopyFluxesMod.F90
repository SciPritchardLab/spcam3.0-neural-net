...
!-----------------------------------------------------------------------
    ...
    ITERATION : do while (itlef <= itmax) 
       ...
       w = exp(-(elai+esai))
       csoilcn = (vkc/(0.13*(z0mg*uaf/1.5e-5)**0.45))*w + csoilc*(1.-w)
       rah(2) = 1./(csoilcn*uaf)
       raw(2) = rah(2) 
  ...
  subroutine Stomata (mpe,  apar,   ei,    ea,   tgcm,   &
    ...
    !
    !-----------------------------------------------------------------------
    ! $Id: CanopyFluxesMod.F90,v 1.1.4.3 2003/08/25 18:05:12 mvertens Exp $
    !-----------------------------------------------------------------------
    ...
    if (apar <= 0.) then          ! night time
       ...
       do iter = 1, niter
          ...
          wc = max(ci-cp,0._r8)*vcmxpot/(ci+awc)*c3psn + vcmxpot*(1.-c3psn)
          we = 0.5*vcmxpot*c3psn + 4000.*vcmxpot*ci/forc_pbot*(1.-c3psn) 
          psn = min(wj,wc,we) 
