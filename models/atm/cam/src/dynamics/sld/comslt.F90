...
#ifdef QVORTDAMP
  real(r8), allocatable :: qtend_qfcst(:,:,:)  ! for comparing to QTEND
  real(r8), allocatable :: dqfcst(:,:,:) ! for bracketing effect of lever
#endif
...
contains
  ...
  subroutine initialize_comslt
    ...
    qfcst (:,:,:,:) = inf
#ifdef QVORTDAMP
    allocate (dqfcst(plond,plev,beglat:endlat))
    dqfcst (:,:,:) = inf
    allocate (qtend_qfcst(plond,plev,beglat:endlat))
    qtend_qfcst (:,:,:) = inf
#endif
    return
