...
! -----------------------------------------------------------------
    ...
    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pes%q_ref2m * wt(pi)   
    end do
    c%ces%pes_a%q_ref2m = pftsum
    ...
    pftsum = 0.0
    do pi=1,npfts
       pftsum = pftsum + c%p(pi)%pef%fsds_vis_d * wt(pi)
    end do
    c%cef%pef_a%fsds_vis_d = pftsum

    pftsum = 0.0
    do pi=1,npfts
       pftsum = pftsum + c%p(pi)%pef%fsds_nir_d * wt(pi)
    end do
    c%cef%pef_a%fsds_nir_d = pftsum

    pftsum = 0.0
    do pi=1,npfts
       pftsum = pftsum + c%p(pi)%pef%fsds_vis_i * wt(pi)
    end do
    c%cef%pef_a%fsds_vis_i = pftsum

    pftsum = 0.0
    do pi=1,npfts
       pftsum = pftsum + c%p(pi)%pef%fsds_nir_i * wt(pi)
    end do
    c%cef%pef_a%fsds_nir_i = pftsum

    pftsum = 0.0
    do pi=1,npfts
       pftsum = pftsum + c%p(pi)%pef%fsr_vis_d * wt(pi)
    end do
    c%cef%pef_a%fsr_vis_d = pftsum

    pftsum = 0.0
    do pi=1,npfts
       pftsum = pftsum + c%p(pi)%pef%fsr_nir_d * wt(pi)
    end do
    c%cef%pef_a%fsr_nir_d = pftsum

    pftsum = 0.0
    do pi=1,npfts
       pftsum = pftsum + c%p(pi)%pef%fsr_vis_i * wt(pi)
    end do
    c%cef%pef_a%fsr_vis_i = pftsum

    pftsum = 0.0
    do pi=1,npfts
       pftsum = pftsum + c%p(pi)%pef%fsr_nir_i * wt(pi)
    end do
    c%cef%pef_a%fsr_nir_i = pftsum

    ! Note - all local noon variables will only be defined at the
    ! pft level - therefore averaging will not be done here
...
! -----------------------------------------------------------------
    ...
    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%fpsn * wt(pi)   
    end do
    c%ccf%pcf_a%fpsn = pftsum

#if (defined BGC)
    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%frm * wt(pi)   
    end do
    c%ccf%pcf_a%frm = pftsum

    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%frmf * wt(pi)   
    end do
    c%ccf%pcf_a%frmf = pftsum

    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%frms * wt(pi)   
    end do
    c%ccf%pcf_a%frms = pftsum

    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%frmr * wt(pi)   
    end do
    c%ccf%pcf_a%frmr = pftsum

    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%frg * wt(pi)   
    end do
    c%ccf%pcf_a%frg = pftsum

    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%dmi * wt(pi)   
    end do
    c%ccf%pcf_a%dmi = pftsum

    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%fco2 * wt(pi)   
    end do
    c%ccf%pcf_a%fco2 = pftsum

    pftsum = 0.0
    do pi=1,npfts 
       pftsum = pftsum + c%p(pi)%pcf%fmicr * wt(pi)   
    end do
    c%ccf%pcf_a%fmicr = pftsum
#endif
