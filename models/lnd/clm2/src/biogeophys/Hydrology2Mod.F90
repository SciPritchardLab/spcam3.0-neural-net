...
!-----------------------------------------------------------------------
    ...
    if (ityplun==istwet .or. ityplun==istice) call WetIceHydrology (c)
    ...
    if (ityplun == istsoil) then
       ...
       do j = 1, nlevsoi
          if (z(j)+0.5*dz(j) <= 0.5) then
             watdry = watsat(j) * (316230./sucsat(j)) ** (-1./bsw(j))
             rwat = rwat + (h2osoi_vol(j)-watdry) * dz(j)
             swat = swat + (watsat(j)    -watdry) * dz(j)
             rz = rz + dz(j)
          end if
       end do
       if (rz /= 0.) then
          ...
          watdry = watsat(1) * (316230./sucsat(1)) ** (-1./bsw(1))
          tsw = h2osoi_vol(1) - watdry
          stsw = watsat(1) - watdry
       end if
       wf = tsw/stsw
    else
       wf = 1.0
