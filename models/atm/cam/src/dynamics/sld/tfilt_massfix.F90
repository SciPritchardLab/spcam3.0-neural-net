...
#ifdef QVORTDAMP
  real(r8) ta_qfcst (plond,plev) ! vapor advection explainable by "qfcst".
  real(r8) ta_qfix (plond,plev) ! vapor advection from "fixer". 
#endif
...
#ifdef QVORTDAMP
           if (m .eq. 1) then
             ta_qfcst(i,k) = (qfcst1(i,k,m) - q3(i,k,m))/ztodt
             ta_qfix(i,k) = dqfx3(i,k,m)
           end if
#endif
        end do
     end do
...
#ifdef QVORTDAMP
  call outfld ('QTENDQFCST',ta_qfcst,plond,lat)
  call outfld ('QTENDQFIX',ta_qfix,plond,lat)
#endif
