...
!-----------------------------------------------------------------------
   ...
   use cloudsimulator, only: isccptab
#if ( defined SPMD || defined COUP_CSM )
   use mpishorthand, only: mpicom, nsend, nrecv, nwsend, nwrecv
#endif
   use shr_sys_mod, only: shr_sys_flush
   use runtime_opts, only: runtime_options, l_analyses
   use analyses, only: analyses_ini
...
#endif
#ifdef QVORTDAMP
   use vorticity_anncycle, only: need_to_read
#endif
!-----------------------------------------------------------------------
   implicit none
...
#if ( defined CRAY ) && ( ! defined PVP )
   write(0,*)'CAM2: cpp token CRAY is defined but PVP is not.'
   write(0,*)'PVP must be defined for optimal performance on this'
   write(0,*)'architecture.  If you really want to run this way,'
   write(0,*)'delete the stop 999 statement in cam.F'
   stop 999
#endif
!
...
!
   if (masterproc) then
      ...
      write(6,*)'$Name:  $ '
      write(6,*)'$Date: 2003/12/18 16:21:31 $'
      write(6,*)'------------------------------------------------------------'
...
#if ( ! defined SPMD && defined TWOD_YZ )
         write(6,*)'FATAL: Twod_yz is incompatible with Spmd - please remake code'
         call endrun ()
#endif
      end if
   end if
...
!
! If pushing model with analysis data, read and process data
!
   if(l_analyses) then
      call analyses_ini
   endif
