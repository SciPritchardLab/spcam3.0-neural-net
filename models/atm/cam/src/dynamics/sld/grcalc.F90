...
!-----------------------------------------------------------------------
!
! $Id: grcalc.F90,v 1.5.8.3 2003/12/15 18:53:04 hender Exp $
! $Author: hender $
!
!-----------------------------------------------------------------------
...
#ifdef QVORTDAMP
  write (6,*) 'QVORTDAMP mods not implemented for this defn of PVP!'
  call endrun
#endif
...
subroutine grcalcs (irow    ,ztodt   ,grts    ,grqs    ,grths   , &
                    grds    ,grus    ,gruhs   ,grvs    ,grvhs   , &
#ifdef QVORTDAMP
                    auxgrus    ,auxgruhs   ,auxgrvs    ,auxgrvhs   , qvortdampfac_equil, qvortnstepspinup, nstep, qvort_dailymean_interference, &
#endif
                    grpss   ,grdpss  ,grpms   ,grpls   ,grtms   , &
                    grtls   ,grqms   ,grqls   )
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
    ...
    use comhd
#ifdef QVORTDAMP
    use vorticity_anncycle, only: vz_anncycle_int, vz_dailymean 
#endif
    implicit none
...
#ifdef QVORTDAMP
    real(r8), intent(out) :: auxgrus(2*maxm,plev)  ! sum(n) of z(n,m)*H(n,m)*a/(n(n+1))
    real(r8), intent(out) :: auxgruhs(2*maxm,plev) ! sum(n) of K(2i)*z(n,m)*H(n,m)*a/(n(n+1))
    real(r8), intent(out) :: auxgrvs(2*maxm,plev)  ! sum(n) of d(n,m)*H(n,m)*a/(n(n+1))
    real(r8), intent(out) :: auxgrvhs(2*maxm,plev) ! sum(n) of K(2i)*d(n,m)*H(n,m)*a/(n(n+1))
    real (r8), intent (in) :: qvortdampfac_equil
   real(r8), intent(in) :: qvortnstepspinup
   integer, intent(in) :: nstep
   logical, intent(in) :: qvort_dailymean_interference
#endif
...
#ifdef QVORTDAMP
    real(r8) auxgru1s (2*maxm)      ! sum(n) of d(n,m)*P(n,m)*m*a/(n(n+1))
    real(r8) auxgruh1s(2*maxm)      ! sum(n) of K(2i)*d(n,m)*P(n,m)*m*a/(n(n+1))
    real(r8) auxgrv1s (2*maxm)      ! sum(n) of z(n,m)*P(n,m)*m*a/(n(n+1))
    real(r8) auxgrvh1s(2*maxm)      ! sum(n) of K(2i)*z(n,m)*P(n,m)*m*a/(n(n+1))
    real(r8) qvortdampfac,mythresh
    real(r8) :: loc_distance,loc_nvalue,loc_term1,loc_term2,loc_term3
    real(r8) :: myvzr,myvzi
#endif
...
! Compute alpn and dalpn
!
#ifdef QVORTDAMPDEBUG
     integer :: kvals(5), mvals(5),nvals(5),kdebug
#endif
...
! Took a fair bit of fiddling to find five independent
! modes that are handled by masterproc but I did it.
kvals(1) = 15
mvals(1) = mlength
nvals(1) = nlen(locm(mvals(1),iam))

kvals(2) = 25
mvals(2) = mlength
nvals(2) = nlen(locm(mvals(2),iam))
...
!
    ...
    auxgrvhs(:,:) = 0.
      ...
      if (nstep .ge. 2) then        
        qvortdampfac = (qvortdampfac_equil-1.)/(1. + exp (-16.*((DBLE(nstep)-1.)/qvortnstepspinup - 0.5))) + 1.
      else
        qvortdampfac = 1.
      end if
      mythresh=2. ! Needs to be a number greater than 1. Bigger = more restrictive m-close-to-n rule
#endif
    grpss(:)   = 0.
    grdpss(:)   = 0.
    grpms(:)   = 0.
...
#ifdef QVORTDAMP
             auxgru1s (2*lm-1) = auxgru1s (2*lm-1) + d(ir,k)*alpn(mr+n)
             auxgru1s (2*lm  ) = auxgru1s (2*lm  ) + d(ii,k)*alpn(mr+n)
!
             auxgruh1s(2*lm-1) = auxgruh1s(2*lm-1) - d(ir,k)*tmp
             auxgruh1s(2*lm  ) = auxgruh1s(2*lm  ) - d(ii,k)*tmp
!

             if (qvort_dailymean_interference) then
                ! Alpha modulates daily mean anomalies with respect to daily mean annual cycle.
                myvzr = vz(ir,k) - vz_dailymean(ir,k) + vz_anncycle_int(ir,k) + qvortdampfac*(vz_dailymean(ir,k) - vz_anncycle_int(ir,k))
                myvzi = vz(ii,k) - vz_dailymean(ii,k) + vz_anncycle_int(ii,k) + qvortdampfac*(vz_dailymean(ii,k) - vz_anncycle_int(ii,k))
             else
                ! Alpha modulates instantaneous anomalies with respect to daily mean annual cycle.
                myvzr=  vz_anncycle_int(ir,k) + qvortdampfac*(vz (ir,k) - vz_anncycle_int(ir,k))
                myvzi = vz_anncycle_int(ii,k) + qvortdampfac*(vz(ii,k) - vz_anncycle_int(ii,k))
             end if

             auxgrv1s (2*lm-1) = auxgrv1s (2*lm-1) + myvzr*alpn(mr+n)
             auxgrv1s (2*lm  ) = auxgrv1s (2*lm  ) + myvzi*alpn(mr+n)
!
             auxgrvh1s(2*lm-1) = auxgrvh1s(2*lm-1) - myvzr*tmp
             auxgrvh1s(2*lm  ) = auxgrvh1s(2*lm  ) - myvzi*tmp

#endif
#ifdef QVORTDAMPDEBUG
             do kdebug=1,5
               if (masterproc .and. irow .eq. 1 .and. k .eq. kvals(kdebug) .and. lm .eq. mvals(kdebug) .and. n .eq. nvals(kdebug)) then
                 write (1029,'(4(I15))') nstep,kvals(kdebug), mvals(kdebug), nvals(kdebug)
                 write (1029,'(4(E15.5))') vz(ir,k), vz_dailymean(ir,k), vz_anncycle_int(ir,k), myvzr
                 write (1029,'(4(E15.5))') vz(ii,k), vz_dailymean(ii,k), vz_anncycle_int(ii,k), myvzi
               end if
             end do
#endif
          end do
...
!
             ...
             auxgrus (2*lm-1,k) = auxgrus (2*lm-1,k) + myvzr*dalpn(mr+n)
             auxgrus (2*lm  ,k) = auxgrus (2*lm  ,k) + myvzi*dalpn(mr+n)
!
             auxgruhs(2*lm-1,k) = auxgruhs(2*lm-1,k) - myvzr*tmp
             auxgruhs(2*lm  ,k) = auxgruhs(2*lm  ,k) - myvzi*tmp
!
             auxgrvs (2*lm-1,k) = auxgrvs (2*lm-1,k) - d(ir,k)*dalpn(mr+n)
             auxgrvs (2*lm  ,k) = auxgrvs (2*lm  ,k) - d(ii,k)*dalpn(mr+n)
!
             auxgrvhs(2*lm-1,k) = auxgrvhs(2*lm-1,k) + d(ir,k)*tmp
             auxgrvhs(2*lm  ,k) = auxgrvhs(2*lm  ,k) + d(ii,k)*tmp
#endif
          end do
       end do
!
...
#ifdef QVORTDAMP
          auxgrus (2*lm-1,k) = auxgrus (2*lm-1,k) + auxgru1s (2*lm  )
          auxgruhs(2*lm-1,k) = auxgruhs(2*lm-1,k) + auxgruh1s(2*lm  )
          auxgrus (2*lm  ,k) = auxgrus (2*lm  ,k) - auxgru1s (2*lm-1)
          auxgruhs(2*lm  ,k) = auxgruhs(2*lm  ,k) - auxgruh1s(2*lm-1)
          auxgrvs (2*lm-1,k) = auxgrvs (2*lm-1,k) + auxgrv1s (2*lm  )
          auxgrvhs(2*lm-1,k) = auxgrvhs(2*lm-1,k) + auxgrvh1s(2*lm  )
          auxgrvs (2*lm  ,k) = auxgrvs (2*lm  ,k) - auxgrv1s (2*lm-1)
          auxgrvhs(2*lm  ,k) = auxgrvhs(2*lm  ,k) - auxgrvh1s(2*lm-1)
#endif
...
subroutine grcalca (irow    ,ztodt   ,grta    ,grqa    ,grtha   , &
                    grda    ,grua    ,gruha   ,grva    ,grvha   , &
#ifdef QVORTDAMP
                    auxgrua  ,auxgruha   ,auxgrva    ,auxgrvha   , qvortdampfac_equil, qvortnstepspinup, nstep, qvort_dailymean_interference,  &
#endif
                    grpsa   ,grdpsa  ,grpma   ,grpla   ,grtma   , &
                    grtla   ,grqma   ,grqla   )
...
#ifdef QVORTDAMP
    real(r8), intent(out) :: auxgrua(2*maxm,plev)  ! sum(n) of z(n,m)*H(n,m)*a/(n(n+1))
    real(r8), intent(out) :: auxgruha(2*maxm,plev) ! sum(n) of K(2i)*z(n,m)*H(n,m)*a/(n(n+1))
    real(r8), intent(out) :: auxgrva(2*maxm,plev)  ! sum(n) of d(n,m)*H(n,m)*a/(n(n+1))
    real(r8), intent(out) :: auxgrvha(2*maxm,plev) ! sum(n) of K(2i)*d(n,m)*H(n,m)*a/(n(n+1))
    real(r8), intent(in) :: qvortdampfac_equil
   real(r8), intent(in) :: qvortnstepspinup
   integer, intent(in) :: nstep
   logical, intent(in) :: qvort_dailymean_interference
...
#ifdef QVORTDAMP
    real(r8) auxgru1a (2*maxm)      ! sum(n) of d(n,m)*P(n,m)*m*a/(n(n+1))
    real(r8) auxgruh1a(2*maxm)      ! sum(n) of K(2i)*d(n,m)*P(n,m)*m*a/(n(n+1))
    real(r8) auxgrv1a (2*maxm)      ! sum(n) of z(n,m)*P(n,m)*m*a/(n(n+1))
    real(r8) auxgrvh1a(2*maxm)      ! sum(n) of K(2i)*z(n,m)*P(n,m)*m*a/(n(n+1))
    real(r8) qvortdampfac,mythresh
    real(r8) :: loc_distance,loc_nvalue,loc_term1,loc_term2,loc_term3
    real (r8) :: myvzr,myvzi
#endif
...
!
             ...
             auxgrua (2*lm-1,k) = auxgrua (2*lm-1,k) + myvzr*dalpn(mr+n)
             auxgrua (2*lm  ,k) = auxgrua (2*lm  ,k) + myvzi*dalpn(mr+n)
!
             auxgruha(2*lm-1,k) = auxgruha(2*lm-1,k) - myvzr*tmp
             auxgruha(2*lm  ,k) = auxgruha(2*lm  ,k) - myvzi*tmp
!
             auxgrva (2*lm-1,k) = auxgrva (2*lm-1,k) - d(ir,k)*dalpn(mr+n)
             auxgrva (2*lm  ,k) = auxgrva (2*lm  ,k) - d(ii,k)*dalpn(mr+n)
!
             auxgrvha(2*lm-1,k) = auxgrvha(2*lm-1,k) + d(ir,k)*tmp
             auxgrvha(2*lm  ,k) = auxgrvha(2*lm  ,k) + d(ii,k)*tmp
...
!
             ...
             auxgru1a (2*lm-1) = auxgru1a (2*lm-1) + d(ir,k)*alpn(mr+n)
             auxgru1a (2*lm  ) = auxgru1a (2*lm  ) + d(ii,k)*alpn(mr+n)
!
             auxgruh1a(2*lm-1) = auxgruh1a(2*lm-1) - d(ir,k)*tmp
             auxgruh1a(2*lm  ) = auxgruh1a(2*lm  ) - d(ii,k)*tmp
!

             auxgrv1a (2*lm-1) = auxgrv1a (2*lm-1) + myvzr*alpn(mr+n)
             auxgrv1a (2*lm  ) = auxgrv1a (2*lm  ) + myvzi*alpn(mr+n)
!
             auxgrvh1a(2*lm-1) = auxgrvh1a(2*lm-1) - myvzr*tmp
             auxgrvh1a(2*lm  ) = auxgrvh1a(2*lm  ) - myvzi*tmp
...
#ifdef QVORTDAMP
          auxgrua (2*lm-1,k) = auxgrua (2*lm-1,k) + auxgru1a (2*lm  )
          auxgruha(2*lm-1,k) = auxgruha(2*lm-1,k) + auxgruh1a(2*lm  )
          auxgrua (2*lm  ,k) = auxgrua (2*lm  ,k) - auxgru1a (2*lm-1)
          auxgruha(2*lm  ,k) = auxgruha(2*lm  ,k) - auxgruh1a(2*lm-1)
          auxgrva (2*lm-1,k) = auxgrva (2*lm-1,k) + auxgrv1a (2*lm  )
          auxgrvha(2*lm-1,k) = auxgrvha(2*lm-1,k) + auxgrvh1a(2*lm  )
          auxgrva (2*lm  ,k) = auxgrva (2*lm  ,k) - auxgrv1a (2*lm-1)
          auxgrvha(2*lm  ,k) = auxgrvha(2*lm  ,k) - auxgrvh1a(2*lm-1)
#endif
!
! Derivatives
!
