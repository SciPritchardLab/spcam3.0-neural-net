...
!-----------------------------------------------------------------------
!
! $Id: settau.F90,v 1.6.6.1 2002/06/15 13:47:52 erik Exp $
! $Author: erik $
!
!-----------------------------------------------------------------------
...
#if ( defined CRAY )
  real(r8) work(plev,2)          ! work array for matrix inverter
  real(r8) zdtr                  ! used by minv
#else
...
!
  ...
  if (nneg/=0) then
     write(6,*)'---------------------------------------------------'
     write(6,*)'SETTAU: UNSTABLE MEAN TEMPERATURE. STOP'
     call endrun
  end if
!
...
!
! Use CRAY PVP routine to invert matrix, otherwise use linpack routines
!  
#if ( defined CRAY )
     call minv(zb,plev,plev,work,zdtr,1.0e-100,0,1)
#else
