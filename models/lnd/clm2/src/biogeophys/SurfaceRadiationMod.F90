...
!-----------------------------------------------------------------------
    ...
    do pi=1,cps%npfts
       ...
       fsds_vis_d = forc_solad(1)
       fsds_nir_d = forc_solad(2)
       fsds_vis_i = forc_solai(1)
       fsds_nir_i = forc_solai(2)
       fsr_vis_d  = albd(1)*forc_solad(1)
       fsr_nir_d  = albd(2)*forc_solad(2)
       fsr_vis_i  = albi(1)*forc_solai(1)
       fsr_nir_i  = albi(2)*forc_solai(2)
       local_secp1 = ctl%secs + nint((londeg/15.*3600.)/dtime)*dtime
       local_secp1 = mod(local_secp1,86400)
       if (local_secp1 == 43200) then
         fsds_vis_d_ln = forc_solad(1)
         fsds_nir_d_ln = forc_solad(2)
         fsr_vis_d_ln = albd(1)*forc_solad(1)
         fsr_nir_d_ln = albd(2)*forc_solad(2)
       else
         fsds_vis_d_ln = spval
         fsds_nir_d_ln = spval
         fsr_vis_d_ln = spval
