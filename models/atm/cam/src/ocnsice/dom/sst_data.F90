...
!
! $Id: sst_data.F90,v 1.9.2.7 2003/12/12 22:54:59 rosinski Exp $
!
!----------------------------------------------------------------------- 
...
!----------------------------------------------------------------------- 
  ...
  integer, parameter :: totsstsz=9612
...
!
  if (aqua_planet) then
     ...
     if (masterproc) then
        ...
        pi         = 4.*atan(1.)
        pio180     = pi/180.
        if(sst_option .lt. 1 .or. sst_option .gt. 11) then
        ...
        if(sst_option == 11) then   ! CSU GCM aqua-planet; Marat Khairoutdinov
           if (aqua_AndKua .or. aqua_3KW1) then 
			...
			IF (clat(j)/pio180 .lt. -60. .or. clat(j)/pio180.gt. 60.) THEN
              			zeta = 1.0
           		ELSEIF (clat(j)/pio180 .ge. -60. .and. clat(j)/pio180.lt. 5.) THEN
              			zeta = (sin(3.141593*((clat(j)/pio180- 5.) / 130.)))**2
           		ELSEIF (clat(j)/pio180.ge. 5. .and. clat(j)/pio180.le. 60.) THEN
            ...
            if (aqua_3KW1) then 
             ...
             do j=1,plat
               do i=1,nlon(j)
                 sst_aqua(i,j) = aqua_uniform_sst_degC
                 xvar(i,j,1) = sst_aqua(i,j)
               end do
             end do 
           else
             write (6,*) 'PRITCH HEY! No aquaplanet option set.'
             call endrun
           end if
        end if
