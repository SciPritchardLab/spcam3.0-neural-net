...
!-----------------------------------------------------------------------
   ...
   use aerosols, only: idxBG, idxSUL, idxSSLT, idxOCPHO, idxBCPHO, idxOCPHI, idxBCPHI, &
     idxDUSTfirst, numDUST, naer_all
   ...
   parameter ( nspint = 19 )
   parameter ( naer_groups = 6 )    ! current groupings are sul, sslt, all carbons, all dust, and all aerosols
...
#ifdef CRM
   real(r8), intent(out) :: fsutoa(pcols)    ! Flux shortwave upwelling TOA
   real(r8), intent(out) :: fsutoac(pcols)   ! Clear sky Flux shortwave upwelling TOA
#endif
...
! 
               ...
               if(rhtrunc.lt.0._r8) call endrun
...
!
               ...
               if (g_tot.gt.1._r8) call endrun
               if (g_tot.lt.-1._r8) call endrun
               ga(i,k)     = g_tot
               if (f_tot.gt.1._r8) write(6,*)"f_tot > 1"
               if (f_tot.lt.0._r8) write(6,*)"f_tot < 0"
               if (f_tot.gt.1._r8) call endrun
               if (f_tot.lt.0._r8) call endrun
               fa(i,k)     = f_tot
               ...
               aertau(i,ns,6) = aertau(i,ns,6) + tau_tot
               ...
               aerssa(i,ns,6) = aerssa(i,ns,6) + tau_w_tot
               ...
               aerasm(i,ns,6) = aerasm(i,ns,6) + tau_w_g_tot
               ...
               aerfwd(i,ns,6) = aerfwd(i,ns,6) + tau_w_f_tot
...
#ifdef CRM
            fsutoa(i)  = fsutoa(i)+solflx*fluxup(0)
#endif
!
