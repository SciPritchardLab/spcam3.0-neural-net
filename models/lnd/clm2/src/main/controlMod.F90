...
!#if (definded SPMD)
!  private :: control_spmd  ! broadcast control information
!#endif
! 
! PRIVATE TYPES:
...
!
! !DESCRIPTION: 
! Initialize clm2 run control information
!
! !USES:
...
! ----------------------------------------------------------------------
    ...
    namelist /clmexp/  &
         ...
         conchk, irad, wrtdia, csm_doflxave, rtm_nsteps, pertlim 
...
!       rpntpath = trim(homedir)//'/lnd.'//trim(caseid)//'.rpointer'
       rpntpath = './lnd.'//trim(caseid)//'.rpointer' ! MSP changed to local directory
    endif
...
!-----------------------------------------------------------------------
    ...
    call mpi_bcast (calendar, 32, MPI_CHARACTER, 0, mpicom, ier)
    ...
    call mpi_bcast (conchk, 1, MPI_LOGICAL, 0, mpicom, ier)
