...
! !USES:
  ...
  use clm_varcon, only : bdsno, istice, istwet, istsoil, denice, denh2o, spval, zlnd, sb 
...
!-----------------------------------------------------------------------
  ...
  if (readini) then
     ...
     do ci = cols1d%beg,cols1d%end
        ...
        do pi = 1,cps%npfts 
           ...
           pes => p%pes
           pes%t_veg = 283._r8
           p%pef%eflx_lwrad_out = sb*(ces%t_grnd)**4
     ...
     do ci = cols1d%beg,cols1d%end
        ...
        if (.not. lps%lakpoi) then  !not lake
           ! volumetric water
           do j = 1,nlevsoi
              if (lps%itype == istsoil) then 
                 cws%h2osoi_vol(j) = 0.3_r8
              else
                 cws%h2osoi_vol(j) = 1.0_r8
              endif
              cws%h2osoi_vol(j) = min(cws%h2osoi_vol(j),cps%watsat(j))
           end do
  ...
  do ci = cols1d%beg,cols1d%end
     ...
     ! Determine variables needed for albedo subroutines
     call EcosystemDyn(c, doalb, .false.)	

     ! Fraction of soil covered by snow (Z.-L. Yang U. Texas)  
     !cps%frac_sno = tanh(cps%snowdp / (2.5 * zlnd))
