...
! fluxes/quantities over sea ice only
  ...
  real(r8) :: icetime(pcols)       ! percent of time grid cell has any ice (diagnostic)
  real(r8) :: flwup(pcols)
...
! if frzmlt >0 grow new ice
     call init_frzmlt (sst(i), tbot(i), frzmlt(i), aice(i), hi(i), hs, &
                       Tsfc, Tiz, saltz, Fbot, frazil(i), Rside, dtime)
...
! Hence the full model energy conservations is a bit worse 
      fatm2ice(i) = fswabs + flwdabs + flwup(i) + shflxice(i)
!      write(6,*)'i,c,aice1=',i,c,aice(i)
!      write(6,*)'i,c,fswabs1=',i,c,fswabs
!      write(6,*)'i,c,lwsum1=',i,c,flwdabs+flwup(i)
...
! Set non-water constituent fluxes to zero
  ...
  call outfld ('ICETIME ', icetime, pcols, c)
  call t_stopf ('seaice')
