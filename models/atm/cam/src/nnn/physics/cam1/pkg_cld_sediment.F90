...
! Local variables
    ...
    real (r8) :: vfall(pcols)                       ! settling velocity of cloud particles (m/s)
    real (r8) :: icice(pcols)                       ! in cloud ice water content (kg/kg)
    real (r8) :: iciwc(pcols)                       ! in cloud ice water content in g/m3
    real (r8) :: icefac
    real (r8) :: logiwc
...
!-----------------------------------------------------------------------
    ...
    call reltab(1, ncol, t, landfrac, landm, icefrac, rel, snowh)
...
! oldway
             ...
             vfall(i) = vland*landfrac(i) + vocean*(1.-landfrac(i))
...
! newway
             if (rel(i,k) < 40. ) then
                vfall(i) = 2./9. * rhoh2o * gravit * rel(i,k)**2 / eta  * 1.e-12  ! micons^2 -> m^2
             else
                vfall(i) = v40 + vslope * (rel(i,k)-r40)      ! linear above 40 microns
             end if
             ! convert the fall speed to pressure units
             ! but do not apply the traditional
             ! negative convention for pvel.
!             pvliq2(i,k+1) = vfall(i) * rho(i,k)*gravit        ! meters/sec to pascals/sec
             pvliq(i,k+1) = vfall(i) * rho(i,k)*gravit        ! meters/sec to pascals/sec
#endif
          end if
       end do
    end do
...
! get effective radius
       call reitab(1, ncol, t, rei)

       do k = 1,pver
          do i = 1,ncol
             if (cloud(i,k) > 0. .and. cldice(i,k) > 0.) then
                if (rei(i,k) < 40. ) then
                   vfall(i) = 2./9. * rhoh2o * gravit * rei(i,k)**2 / eta  * 1.e-12  ! micons^2 -> m^2
                else
                   vfall(i) = v40 + vslope * (rei(i,k)-r40)      ! linear above 40 microns
                end if
!!$                print *, t(i,k), rei(i,k), vfall(i)*100.
! convert the fall speed to pressure units, but do not apply the traditional
! negative convention for pvel.
                pvice(i,k+1) = vfall(i) * rho(i,k)*gravit        ! meters/sec to pascals/sec
             end if
          end do
       end do
...
! compute the in-cloud ice concentration (kg/kg)
             icice(i) = cldice(i,k) / cloud(i,k)

! compute the ice water content in g/m3
             iciwc(i) = icice(i) * rho(i,k) * 1.e3

! set the fall velocity (cm/s) to depend on the ice water content in g/m3,
             if (iciwc(i) > lbound) then ! need this because of log10
                logiwc = log10(iciwc(i))
!          Median - 
                vfall(i) = 128.64 + 53.242*logiwc + 5.4795*logiwc**2
!          Average - 
!!$             vfall(i) = 122.63 + 44.111*logiwc + 4.2144*logiwc**2
             else
                vfall(i) = 0.
             end if
...
! calculated above at 2*icritc
             if (icice(i) <= icritc) then
                vfall(i) = vice_small
             else if(icice(i) < 2*icritc) then
                icefac = (icice(i)-icritc)/icritc
                vfall(i) = vice_small * (1.-icefac) + vfall(i) * icefac
             end if

! bound the terminal velocity of ice particles at high concentration
             vfall(i) = min(100.0_r8, vfall(i))
