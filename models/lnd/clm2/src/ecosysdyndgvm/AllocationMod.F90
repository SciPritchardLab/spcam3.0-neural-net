...
!
! !INTERFACE:
  subroutine Allocation (pdgvepc   , nind     , bm_inc   , &
                         tree     , height   , sla      , &
...
! !ARGUMENTS:
    implicit none
    type(pft_dgvepc_type),target, intent(in) :: pdgvepc
...
!-----------------------------------------------------------------------
    ...
    ! assign allocation parameters from the dgvm epc structure
    ...
    if (present) then
       ...
       if (tree) then
          ...
          lmtorm = init_lmtorm !slevis: eliminate influence from soil H2O
