...
! !USES:
    ...
    use pftvarcon, only : noveg         !vegetation type (PFT) constants
...
!-----------------------------------------------------------------------
    ...
    if (masterproc) then
       ...
       allocate(pft(lsmlon,lsmlat,maxpatch_pft), &
                pctpft(lsmlon,lsmlat,maxpatch_pft), &
                pctlnd_pft(lsmlon,lsmlat), &
                landfrac_pft(lsmlon,lsmlat), stat=ier)
       if (ier/=0) then
          write(6,*)'MKSRFDATMOD: pft,pctpft,pctlnd_pft, landfrac_pft allocation error'
          call endrun
       endif
    ...
    if (mksrf_offline_fgrid /= ' ') then
       ...
       allocate(mlai (lsmlon,lsmlat,maxpatch_pft,12), &
                msai (lsmlon,lsmlat,maxpatch_pft,12), &
                mhgtt(lsmlon,lsmlat,maxpatch_pft,12), &
                mhgtb(lsmlon,lsmlat,maxpatch_pft,12), stat=ier)
       if (ier/=0) then
          write(6,*)'MKSRFDATMOD: mlai, msai, mhgtt, mhgtb allocation error'
          call endrun
       endif
...
!!$       end do
       ...
       call surfwrt(fsurdat, pft, pctpft, landfrac_pft, mlai, msai, mhgtt, mhgtb)
       ...
       deallocate(pft, pctpft, pctlnd_pft, landfrac_pft)
