#! /usr/bin/csh -f
# make sure to load intelmpi before running this script (should be version >=2021.3 for Forpy compatiility)
...
echo $camroot
#exit(0)
cp $camroot/bld/Makefile.bridges2-forpy $camroot/bld/Makefile
...
set wrkdir       = $PROJECT/test_nov3_nn_forpy
...
# for NN run activate this version:
    $cfgdir/configure_mmf -fflags "-DCLOUDBRAIN -DBRAINDEBUG -DHDEBUG" -fc mpiifort -cc cc -spmd -smp -dyn sld -res 64x128 -pcols 8 -nlev 30 -cam_exedir $rundir -mpi_inc $MPICH_DIR/intel64/include -mpi_lib $MPICH_DIR/intel64/lib/release || echo "configure failed" && exit 1
