...
#endif
   use runtime_opts, only: l_analyses, tau_ps, tau_q, tau_t, tau_u, tau_v, nudge_dse_not_T, subregional_moisture_lever,subregional_moisture_constraint ! pritch
...
! energy checking variables
   ...
   real (r8) u_ls (pcols,pver)
   real (r8) v_ls (pcols,pver)
   ...
   real(r8) cld_crm(crm_nx, crm_ny, crm_nz)
   real(r8) cliqwp_crm(crm_nx, crm_ny, crm_nz)
   real(r8) cicewp_crm(crm_nx, crm_ny, crm_nz)
   real(r8) rel_crm(crm_nx, crm_ny, crm_nz)
   real(r8) rei_crm(crm_nx, crm_ny, crm_nz)
   real(r8) emis_crm(crm_nx, crm_ny, crm_nz)
...
! analysis nudging stuff:
   ...
   real(r8) :: aux_s_a   (pcols,pver)            ! dry static energy of analyses
   real(r8) :: s_tmp (pcols,pver)            ! dry static energy of analyses
   real(r8) ps_local(pcols)
   real(r8) dpdt(pcols)
   logical  lpsntenl
   ...
   do k=1,pver
      do i=1,ncol
         if (nstep > lons(i)) then
            fakefld(i,k) = 1.d0/nstep
         else
            fakefld(i,k) = fillvalue
         end if
      end do
   end do
...
!       print*, 'prec_zmc=',prec_zmc(i)*86400.*1000., int(cnt(i)), state%zm(i,int(cnt(i)))
       call convmom(state%zm(i,:),state%zi(i,:),state%pdel(i,:),state%u(i,:),cape, cmfmc(i,:),&
                    state%zm(i,int(cnt(i))), pver, ptend%u(i,:))  
       call convmom(state%zm(i,:),state%zi(i,:),state%pdel(i,:),state%v(i,:),cape, cmfmc(i,:),&
                    state%zm(i,int(cnt(i))), pver, ptend%v(i,:))  
     end if
   end do
   ptend%lu    = .TRUE.
   ptend%lv    = .TRUE.
...
!        i = 10
!        write(22,*) &
!            state%t(i,:),   state%q(i,:,:ppcnst), pblht(i),   &
!            state%zm(i,:), state%phis(i),    state%zi(i,:), tpert(i), qpert(i,:), &
!            state%pmid(i,:),   state%pint(i,:),  state%pdel(i,:), ztodt, &
!            state%omega(i,:),tcwat(i,:), qcwat(i,:), lcwat(i,:), cld(i,:),&
!            landfrac(i),snowh(i),icefrac(i),landm(i),ts(i), sst(i)
!
!   end if
...
!        i = 10
!        write(22,*) &
!        state%t(i,:),   state%q(i,:,:), cnt(i),     cnb(i),cmfmc(i,:),   cmfmc2(i,:),zdu(i,:),dlf(i,:)
!   end if
...
!
   ...
   do m=1,ppcnst
      ...
      call outfld('DQCOND  ',dqcond(1,1,m),pcols   ,lchnk   )
   end do
...
! Compute cloud water/ice paths and optical properties for input to radiation
      ...
      call param_cldoptics_calc(1, ncol, state, cld, landfrac, landm,icefrac, &
...
!
      ...
      call radctl (lchnk, 1, ncol, lwup, emis, state%pmid,             &
...
! radctl.
!
      call cldsav (lchnk, 1, ncol, cld, state%pmid, cltot, &
...
#endif
      if (doisccp) then
         call ccm_isccp(lchnk, ncol, state%pmid, state%pint, state%q, &
                        state%t, ts, concld,   &
...
!
	...
	wnd = sqrt(state%u(i,pver)**2 + state%v(i,pver)**2)
        bflx(i) = shf(i)/cpair + 0.61*state%t(i,pver)*lhf(i)/latvap
...
! end 
	       ...
	       gicewp(i,:), gliqwp(i,:),     &
               ...
               nbreak, doinitial,  ocnfrac(i), wnd, tau00, bflx(i), taux_crm(i), tauy_crm(i), z0m(i), &
               timing_factor(i),u_ls(i,:),v_ls(i,:) &
               )
...
!          Compute liquid and ice water paths for a given CRM column
            do m=1,crm_nz
               k = pver-m+1
               trad(i,k) = t_rad(ii,jj,m)
               qvrad(i,k,1) = qv_rad(ii,jj,m)
               qtot = qc_rad(ii,jj,m) + qi_rad(ii,jj,m)
               if(qtot.gt.1.e-9) then
                 fice(i,k) = qi_rad(ii,jj,m)/qtot
                 cldn(i,k) = 0.99_r8
		 cld_crm(ii,jj,m)=0.99_r8
                 cicewp(i,k) = qi_rad(ii,jj,m)*state%pdel(i,k)/gravit*1000.0 &
		           / max(0.01_r8,cldn(i,k)) ! In-cloud ice water path.
                 cliqwp(i,k) = qc_rad(ii,jj,m)*state%pdel(i,k)/gravit*1000.0 &
		 ...
		 cld_crm(ii,jj,m)=0.
	       ...
	       cliqwp_crm(ii,jj,m)=cliqwp(i,k)
	       cicewp_crm(ii,jj,m)=cicewp(i,k)
            end do

!           Cloud water and ice particle sizes
            call cldefr(lchnk, i, i, landfrac, state%t, rel, rei, state%ps, state%pmid, landm, icefrac, snowh)

!           Cloud emissivity.
            call cldems(lchnk, i, i, cwp, fice, rei, emis)

            if(doisccp)then
             do m=1,crm_nz
               k = pver-m+1
	       rel_crm(ii,jj,m)=rel(i,k)
	       rei_crm(ii,jj,m)=rei(i,k)
	       emis_crm(ii,jj,m)=emis(i,k)
             end do
	    endif

            call cldovrlap(lchnk,i,i,state%pint,cldn,nmxrgn,pmxrgn)

!
!   Compute radiation:
   call t_startf ('crmrad_call')

            call radctl (lchnk, i, i, lwup, emis, state%pmid, &
                state%pint, state%lnpmid, state%lnpint, trad, qvrad, &
                cldn, cicewp, cliqwp, coszrs, asdir, asdif, &
                aldir, aldif, pmxrgn, nmxrgn, &
                fsns_crm(1,ii,jj), fsnt_crm(1,ii,jj), flns_crm(1,ii,jj), flnt_crm(1,ii,jj), &
                qrs_tmp, qrl_tmp, flwds_crm(1,ii,jj), rel, rei,  &
                sols_crm(1,ii,jj), soll_crm(1,ii,jj), solsd_crm(1,ii,jj), solld_crm(1,ii,jj),  &
                landfrac, state%zm, state, fsds_crm(1,ii,jj), &
                fsntoa_crm(1,ii,jj)  ,fsntoac_crm(1,ii,jj) ,fsdsc_crm(1,ii,jj),   &
		flwdsc_crm(1,ii,jj),fsntc_crm(1,ii,jj) ,fsnsc_crm(1,ii,jj), &
                fsutoa_crm(1,ii,jj) ,fsutoac_crm(1,ii,jj) ,flut_crm(1,ii,jj) , &
		flutc_crm(1,ii,jj) ,flntc_crm(1,ii,jj) ,flnsc_crm(1,ii,jj) ,solin_crm(1,ii,jj), &
                .false., .false.,.true.,.true.)
   call t_stopf('crmrad_call')
		
            do m=1,crm_nz
               k = pver-m+1
               qrs_crm(i,ii,jj,m) = (qrs_tmp(i,k)) / cpair
               qrl_crm(i,ii,jj,m) = (qrl_tmp(i,k)) / cpair
            end do

            if(ii.eq.1.and.jj.eq.1) then
              stat_buffer(i,10) = stat_buffer(i,10) + fsds_crm(i,ii,jj)
              stat_buffer(i,11) = stat_buffer(i,11) + flwds_crm(i,ii,jj)
              stat_buffer(i,12) = stat_buffer(i,12) + fsutoa_crm(i,ii,jj)
              stat_buffer(i,13) = stat_buffer(i,13) + flut_crm(i,ii,jj)
              stat_buffer(i,14) = stat_buffer(i,14) + fsdsc_crm(i,ii,jj)
              stat_buffer(i,15) = stat_buffer(i,15) + fsutoac_crm(i,ii,jj)
            end if

            if(mm.ne.nbreak) then

             qrs(i,:) = qrs(i,:) + qrs_tmp(i,:)  * state%pdel(i,:)
             qrl(i,:) = qrl(i,:) + qrl_tmp(i,:)  * state%pdel(i,:)
             solin(i) = solin(i) + solin_crm(i,ii,jj)
             fsnt(i) = fsnt(i) + fsnt_crm(i,ii,jj)
             fsns(i) = fsns(i) + fsns_crm(i,ii,jj)
             fsds(i) = fsds(i) + fsds_crm(i,ii,jj)
             fsdsc(i) = fsdsc(i) + fsdsc_crm(i,ii,jj)
             fsutoa(i) = fsutoa(i) + fsutoa_crm(i,ii,jj)
             fsutoac(i) = fsutoac(i) + fsutoac_crm(i,ii,jj)
             fsntc(i) = fsntc(i) + fsntc_crm(i,ii,jj)
             fsnsc(i) = fsnsc(i) + fsnsc_crm(i,ii,jj)
             fsntoa(i) = fsntoa(i) + fsntoa_crm(i,ii,jj)
             fsntoac(i) = fsntoac(i) + fsntoac_crm(i,ii,jj)
             sols(i) = sols(i) + sols_crm(i,ii,jj)
             soll(i) = soll(i) + soll_crm(i,ii,jj)
             solsd(i) = solsd(i) + solsd_crm(i,ii,jj)
             solld(i) = solld(i) + solld_crm(i,ii,jj)
             flut(i) = flut(i) + flut_crm(i,ii,jj)
             flutc(i) = flutc(i) + flutc_crm(i,ii,jj)
             flnt(i) = flnt(i) + flnt_crm(i,ii,jj)
             flns(i) = flns(i) + flns_crm(i,ii,jj)
             flntc(i) = flntc(i) + flntc_crm(i,ii,jj)
             flnsc(i) = flnsc(i) + flnsc_crm(i,ii,jj)
             flwds(i) = flwds(i) + flwds_crm(i,ii,jj)
             flwdsc(i) = flwdsc(i) + flwdsc_crm(i,ii,jj)
...
!
        ...
        if (doisccp) then
            call crm_isccp (state%pmid(i,:), state%pint(i,:), qv_rad, t_rad, ts(i),    &
                cld_crm, cliqwp_crm, cicewp_crm, rel_crm, rei_crm, emis_crm, coszrs(i),    &
        ...
        coef = 1._r8/dble(nbreak*crm_nx*crm_ny)
        ...
        lwcf(i) = flutc(i) - flut(i)
        swcf(i) = fsntoa(i) - fsntoac(i)
        stat_buffer(i,10:15) = stat_buffer(i,10:15)/dble(nbreak)
...
! to this #ifdef SPMOMTRANS, now linked to CRM-computed ptend%u
   ...
   call outfld ('SPULS   ',u_ls, pcols,lchnk)
   call outfld ('SPVLS   ',v_ls, pcols,lchnk)
   ...
   call outfld ('BFLX',bflx,pcols,lchnk)
...
! End of superparameterization zone.
  ...
  if(l_analyses) then ! ONly do nudging if bndtva was specified in the namelist
      ...
      ptend%name  = "analyses_nudge"
      s_tmp(:ncol,:pver) = state%s(:ncol,:pver)
     if (nudge_dse_not_T) then
       call outfld('MSPSA',s_a,pcols,lchnk)
     else
       do k=1,pver
         aux_s_a (:ncol,k) = cpair*t_a(:ncol,k,lchnk) + gravit*state%zm(:ncol,k) + state%phis(:ncol)
       end do
       call outfld('MSPSA',aux_s_a,pcols,lchnk)
     endif
     call outfld('MSPS',s_tmp,pcols,lchnk)

     if (nudge_dse_not_T) then
      call analyses_nudge(ztodt   ,tau_t   ,ncol   ,pver    ,s_a(1,1,lchnk), &
                         state%s          ,ptend%s       ,ptend%ls)
     else
      call analyses_nudge(ztodt   ,tau_t   ,ncol   ,pver    ,aux_s_a            , &
                         state%s          ,ptend%s       ,ptend%ls)
     end if
      call analyses_nudge(ztodt   ,tau_u   ,ncol   ,pver    ,u_a (1,1,lchnk), &
                         state%u          ,ptend%u       ,ptend%lu)
      call analyses_nudge(ztodt   ,tau_v   ,ncol   ,pver    ,v_a (1,1,lchnk), &
                         state%v          ,ptend%v       ,ptend%lv)
      call analyses_nudge(ztodt   ,tau_q   ,ncol   ,pver    ,q_a (1,1,lchnk), &
                          state%q(1,1,1)   ,ptend%q(1,1,1),ptend%lq(1))

      ps_local(:ncol) =      state%ps(:ncol)
      state%ps(:ncol) = log( state%ps(:ncol) )
      call analyses_nudge(ztodt   ,tau_ps  ,ncol   ,1       ,ps_a(1,  lchnk), &
                          state%ps         ,dpdt       ,lpsntenl   )
      state%ps(:ncol) = state%ps(:ncol) + dpdt(:ncol) * ztodt
      state%ps(:ncol) = exp( state%ps(:ncol) )
      ps_local(:ncol) = ( state%ps(:ncol) - ps_local(:ncol) )/ztodt

  end if

     call outfld('VNTEND  ',ptend%v       ,pcols   ,lchnk   )
      call outfld('UNTEND  ',ptend%u       ,pcols   ,lchnk   )
      call outfld('QNTEND  ',ptend%q(1,1,1),pcols   ,lchnk   )
      call outfld('LPSNTEN ',ps_local      ,pcols   ,lchnk   )
!
! pressure arrays
!
      call plevs0(ncol, pcols, pver, state%ps,   state%pint, &
                  state%pmid, state%pdel)
!
! log(pressure) arrays and Exner function
!
      state%lnpint(:ncol,:pver+1) = log(state%pint(:ncol,:pver+1))
      state%rpdel (:ncol,:pver) = 1./state%pdel (:ncol,:pver)
      state%lnpmid(:ncol,:pver) = log(state%pmid(:ncol,:pver))
      do k=1,pver
         do i=1,ncol
            state%exner (i,k) = (state%pint(i,pver+1) / state%pmid(i,k))**cappa
         end do
      end do

      call physics_update(state, tend, ptend, ztodt)

      s_tmp(:ncol,:pver) = (state%s(:ncol,:pver) - s_tmp(:ncol,:pver))/ztodt
      call outfld('SNTEND  ',s_tmp   ,pcols   ,lchnk   )


  if (subregional_moisture_constraint) then ! pritch
    ptend%name  = "subregional_moisture_nudge"
    ptend%lq(1) = .true.
    ptend%q = 0.
    nstepsperday = int(24.*3600./ztodt)
!    if (nstepsperday .gt. 96) then
!      write (6,*) 'pritch ERROR in subregional mositure nudging, increase array size of moisture buffer for such a small timestep!'
!    endif
!    ifld = pbuf_get_fld_idx('QBFR')
!    qlastdaybuffer  => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk, 1:96)
!    ifld = pbuf_get_fld_idx('QREF')
!    qdaymean_reference  => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk, 1)
    ! Remember the moisture field avaeraged over the preceding day day:
!    if (nstep .le. nstepsperday) then
      ! Fill up the buffer with successive timesteps..
!      qlastdaybuffer(:ncol,:pver,nstep) = state%q(:ncol,:pver,1)
!    else
      ! If the day-long buffer is already full, shift everything to the left to make a moving
      ! one-day-long time window buffer:
!      do k=1,(nstepsperday-1)
!        qlastdaybuffer(:ncol,:pver,k) = qlastdaybuffer(:ncol,:pver,k+1)
!      end do
!      qlastdaybuffer(:ncol,:pver,nstepsperday) = state%q(:ncol,:pver,1)
      ! Calculate the time mean across the moving one-day window:
!      qlastdaymean(:ncol,:pver) = 0.
!      do k=1,(nstepsperday)
!        qlastdaymean(:ncol,:pver) = qlastdaymean(:ncol,:pver) + qlastdaybuffer(:ncol,:pver,k) 
!      end do
!      qlastdaymean(:ncol,:pver) = qlastdaymean(:ncol,:pver)/nstepsperday
!    end if
    ! At the end of day 2, remember the one-day mean q as a referene state
!    if (nstep .eq. nstepsperday*2) then
!      qdaymean_reference(:ncol,:pver) = qlastdaymean ! stored in pbuf
!    end if
    ! For all subsequent steps, nudge q to the reference date
    ! Only interfere with moisture for days 2 and beyond:
!    if (nstep .ge. nstepsperday*2) then

! determine saturation specific humidity, qsat: (mimicking zm_conv.F90)
    call aqsat (state%t    ,state%pmid  ,est    ,qsat    ,pcols   , &
         1, ncol ,pver  ,1       ,pver    )

      do i=1,ncol
      ! Use a location-dependent nudging timescale.
      ! The following formula identifies an equatorial ellipse centered on 115. E
        distance_from_ellipsoid_center  = sqrt((clon(i)*180./3.141592654 - 115.)**2 + ((clat(i)*180./3.141592654)**2)*8.)
      ! Inside the ellipse, constant fractional addition rate
      ! transitions to zero addition rate far from the ellipse. 
      ! NOTE that "0.2" below is the control knob I will eventually want to vary:
        fraction_to_add_over_eight_days = subregional_moisture_lever/( 1+exp(0.3*(distance_from_ellipsoid_center-35.)) )

        if (distance_from_ellipsoid_center .le. 100.) then
          do k=1,pver
            ! Also make sure this routine is never allowed to supersaturate a model level:
            addlimit = max(0._r8, (qsat(i,k)-state%q(i,k,1))/ztodt) ! mimicking zm_conv.F90
            ptend%q(i,k,1) = min(addlimit,fraction_to_add_over_eight_days*(state%q(i,k,1))/8./24./3600.)
          end do
        end if

      ! DEBUGGING:
!        if (abs(clat(i))*180./3.14159 .le. 10. .and. clon(i)*180./3.14159 .ge. 100. .and. clat(i)*180./3.14159 .le. 120.) then
!          write (6,*) 'MDEBUG lon=',clon(i)*180./3.14158,', lat=',clat(i)*180./3.14159,'ptend(iz=28)=',ptend%q(i,28,1),'qlever=',subregional_moisture_lever
!        end if
      end do ! icol
!    end if ! Far enough into sim for nudging to have activated.
    call outfld('QLEVER', ptend%q, pcols, lchnk)
    call physics_update(state, tend, ptend, ztodt) ! recall qneg3 gets called in here
  end if ! if doing subregional moisture nudging
...
#ifdef QRLDAMP
   call accumulate_dailymean_qrl(lchnk,ncol,qrl,nstep)
 ! pritch: artificial amplification/ damping of radiative
 ! heating anomalies with respect to an offline mean annual cycle. 
   call qrl_interference(lchnk,ncol,qrl,clat,newqrl,qrldampfac,qrldamp_equatoronly, qrl_dylat, qrl_critlat_deg, qrl_dailymean_interference, qrldamp_freetroponly, qrl_pbot,qrl_ptop,qrl_dp,state%pmid)
   ptend%name = 'qrldamp'
   do i=1,ncol
     do k=1,pver
       dqrl(i,k) = newqrl(i,k) - qrl(i,k)
     end do
   end do
!   if (masterproc) then
!     write (6,*) '==========='
...
! profiles:
   call physics_update(state,tend,ptend,ztodt)    
   call outfld('DQRL    ',dqrl/cpair ,pcols,lchnk)
 ...
 ! Hence applying "qrl"-scope anomalies (dqrl) to qrl_crm variable means multiplying first by pdel / cpair.
   do jj=1,crm_ny
     do ii=1,crm_nx
       do m=1,crm_nz
         k = pver-m+1
         qrl_crm(i,ii,jj,m) = qrl_crm(i,ii,jj,m) + dqrl(i,k)/cpair*state%pdel(i,k)
       end do
     end do
   end do 
