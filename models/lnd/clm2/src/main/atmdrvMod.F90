       ...
       do gi = 1,clm%mps%ngridcells
          ...
          if (prcxy(i,j) + prcxy(i,j) > 0.) then
             if (a2ls%forc_t > (SHR_CONST_TKFRZ + tcrit)) then
                ...
                if (a2ls%forc_t <= SHR_CONST_TKFRZ) then
                   a2ls%flfall = 0.0
                else if (a2ls%forc_t <= SHR_CONST_TKFRZ+2.) then
                   a2ls%flfall = coefa + coefb*a2ls%forc_t
                else
                   a2ls%flfall = coefa + coefb*(SHR_CONST_TKFRZ+2.)
                endif
#else
                if (a2ls%forc_t <= SHR_CONST_TKFRZ) then
                   a2ls%flfall = 0.
                else if (a2ls%forc_t <= SHR_CONST_TKFRZ+2.) then
                   a2ls%flfall = -54.632 + 0.2*a2ls%forc_t
                else
                   a2ls%flfall = 0.4
                endif
#endif
          ...
          if (pertlim /= 0.0 .and. do_perturb) then
             ...
             a2ls%forc_t = (a2ls%forc_t)*(1. + pertval)
          endif
#endif
       end do
