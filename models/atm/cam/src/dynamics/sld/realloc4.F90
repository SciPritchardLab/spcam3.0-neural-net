...
!-----------------------------------------------------------------------
!
! $Id: realloc4.F90,v 1.4.22.3 2003/12/18 16:22:34 pworley Exp $
! $Author: pworley $
!
!-----------------------------------------------------------------------
...
!
#ifdef QVORTDAMP
  integer, parameter :: nvaraux=13
#else
  integer, parameter :: nvaraux=11
#endif
   real(r8), intent(in)  :: fftbuf_in(2*maxm,nvaraux,plevp,plat) 
                            ! buffer of Fourier coefficients to be reordered
   real(r8), intent(out) :: fftbuf_out(plond,nvaraux,plevp,beglat:endlat) 
                            ! buffer used for in-place FFTs
!
!---------------------------Local workspace-----------------------------
!
