...
!-----------------------------------------------------------------------
    ...
    case('gridcell')
       do gi = 1, grid1d%num
          if (fldv(gi) /= spval .and. grid1d%wtxy(gi)/=0.) then
             i = grid1d%ixy(gi) 
             j = grid1d%jxy(gi) 
    ...
    case('landunit')
       do li = 1, land1d%num
          if (fldv(li) /= spval .and. land1d%wtxy(li)/=0.) then
             i = land1d%ixy(li) 
             j = land1d%jxy(li) 
             if (sumwt(i,j)==0.) fldxy(i,j) = 0.
    ...
    case('column')
       do ci = 1, cols1d%num
          if (fldv(ci) /= spval .and. cols1d%wtxy(ci)/=0.) then
             i = cols1d%ixy(ci) 
             j = cols1d%jxy(ci) 
             if (sumwt(i,j)==0.) fldxy(i,j) = 0.
    ...
    case('pft')
       do pi = 1, pfts1d%num
          if (fldv(pi) /= spval .and. pfts1d%wtxy(pi)/=0.) then
