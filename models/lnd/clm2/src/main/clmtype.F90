...
type pft_estate_type
   ...
   real(r8):: t_ref2m_min      !daily minimum of average 2 m height surface air temperature (K)
   real(r8):: t_ref2m_max      !daily maximum of average 2 m height surface air temperature (K)
   real(r8):: t_ref2m_min_inst !instantaneous daily min of average 2 m height surface air temp (K)
   real(r8):: t_ref2m_max_inst !instantaneous daily max of average 2 m height surface air temp (K)
   real(r8):: q_ref2m          !2 m height surface specific humidity (kg/kg)
...
type pft_eflux_type
   ...
   real(r8):: fsds_vis_d        !incident direct beam vis solar radiation (W/m**2)
   real(r8):: fsds_nir_d        !incident direct beam nir solar radiation (W/m**2)
   real(r8):: fsds_vis_i        !incident diffuse vis solar radiation (W/m**2)
   real(r8):: fsds_nir_i        !incident diffuse nir solar radiation (W/m**2)
   real(r8):: fsr_vis_d         !reflected direct beam vis solar radiation (W/m**2)
   real(r8):: fsr_nir_d         !reflected direct beam nir solar radiation (W/m**2)
   real(r8):: fsr_vis_i         !reflected diffuse vis solar radiation (W/m**2)
   real(r8):: fsr_nir_i         !reflected diffuse nir solar radiation (W/m**2)
   real(r8):: fsds_vis_d_ln     !incident direct beam vis solar radiation at local noon (W/m**2)
   real(r8):: fsds_nir_d_ln     !incident direct beam nir solar radiation at local noon (W/m**2)
   real(r8):: fsr_vis_d_ln      !reflected direct beam vis solar radiation at local noon (W/m**2)
   real(r8):: fsr_nir_d_ln      !reflected direct beam nir solar radiation at local noon (W/m**2)
...
type column_estate_type
   ...
   real(r8):: tssbef(-nlevsno+1:nlevsoi)   !soil/snow temperature before update
...
type gridcell_pstate_type
   ...
   real(r8):: landfrac         !fractional land for this gridcell
   integer :: nlandunits       !number of landunits for this gridcell
   integer :: itype            !gridcell type 
   integer :: ixy              !xy lon index
   integer :: jxy              !xy lat index
...
#if (defined OFFLINE)
   real(r8):: flfall            !fraction of liquid water within falling precipitation
#endif
...
type lnd2atm_state_type
   ...
   real(r8):: albi(numrad)	!surface albedo (diffuse)
#if (defined DUST)
   real(r8):: fv                !land friction velocity (need for dust)
   real(r8):: ram1              !land aerodynamic resistance (need for dust)
#endif
...
type lnd2atm_flux_type
   ...
   real(r8):: fsa 		 !solar radiation absorbed (total) (W/m**2)
#if (defined DUST)
   real(r8):: flxdst(ndst)       !land derived dust flux for ndst bins
#endif
...
type gridcell_type
   ...
   ! pointers to atmospheric state and flux variables 
...
type landunit_type
   ...
   ! pointer to landunits at higher level in hierarchy
   type(gridcell_type), pointer::g      	!pointer to gridcell at higher level
...
type column_type
   ...
   ! pointer to landunit at higher level in hierarchy
   type(landunit_type),pointer::l	    !pointer to landunit at higher level
...
type pft_type
   ! pointer to column at higher level in hierarchy
   type(column_type), pointer::c             !pointer to column at higher level

   ! pointer to atmospheric state and flux variables
   type(atm2lnd_state_type), pointer ::	a2ls !atmospheric state variables required by the land
   type(atm2lnd_flux_type) , pointer ::	a2lf !atmospheric flux variables required by the land
...
type landunit_1d
   ...
   real(r8),pointer :: latdeg(:)   !latitudes in degrees north
   real(r8),pointer :: londeg(:)   !longitudes in degrees east
   integer, pointer :: ityplun(:)  !landunit type (vegetated (includes bare),lake,wetland,glacier,urban)
