...
! !USES:
    ...
    use pftvarcon, only : noveg         !vegetation type (PFT) 
       ...
       do j = 1,lsmlat
          do i = 1,numlon(j)
             ...
             if (cam_numlon(j) /= numlon(j)) then                 
                write(6,*)'CAM numlon array different from surface dataset input' 
                call endrun
             else if (cam_landmask(i,j) /= landmask(i,j)) then         
                write(6,*)'CAM land mask different from surface dataset input at i,j= ',i,j 
                write(6,*)'cam_landmask=',cam_landmask(i,j), '   landmask=',landmask(i,j)
                call endrun
             elseif (cam_landfrac(i,j) /= landfrac(i,j)) then
                write(6,*)'CAM fractional land different from surface dataset input',&
                     ' at i,j= ',i,j
                write(6,*)'cam_landfrac=',cam_landfrac(i,j), '   landfrac=',landfrac(i,j)
#if (defined COUP_CAM)
!                call endrun
#elif (defined COUP_CSM)
                if (cam_landfrac(i,j) > 1.e-8 .and. &
                     abs(cam_landfrac(i,j)-landfrac(i,j)) > 1.e-12) then
                   write(6,*)'CAM fractional land different from surface dataset input ',&
...
!
! !INTERFACE:
  subroutine surfwrt(fname, pft, pctpft, landfrac_pft, mlai, msai, mhgtt, mhgtb)
!
! !DESCRIPTION: 
...
! !ARGUMENTS:
    ...
    real(r8), intent(in) :: landfrac_pft(lsmlon,lsmlat)          !PFT data fractional land 
...
! !LOCAL VARIABLES:
    ...
    integer landfrac_pft_id      !variable id
...
!-----------------------------------------------------------------------
    ...
    call wrap_put_att_text (ncid, NF_GLOBAL, 'History_Log', trim(str))
    ...
    str = '$Id: surfFileMod.F90,v 1.13.2.7 2003/06/13 16:21:15 hender Exp $'
    ...
    name = 'land fraction from pft dataset'
    unit = 'unitless'
    call wrap_def_var (ncid, 'LANDFRAC_PFT', nf_double, 2, dim2_id, landfrac_pft_id)
    call wrap_put_att_text (ncid, landfrac_pft_id, 'long_name', name)
    call wrap_put_att_text (ncid, landfrac_pft_id, 'units'    , unit)
    ...
    call wrap_put_var_realx (ncid, landfrac_pft_id, landfrac_pft)
