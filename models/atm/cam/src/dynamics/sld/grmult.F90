...
#ifdef QVORTDAMP
                   u3aux,v3aux,u3m1aux,v3m1aux,u3sldaux,v3sldaux, &
#endif
                   parrsld ,nlon    )
!-----------------------------------------------------------------------
!
...
#ifdef QVORTDAMP
  real(r8), intent(in)   :: u3aux      (plond,plev)  ! zonal wind
  real(r8), intent(in)   :: v3aux      (plond,plev)  ! meridional wind
  real(r8), intent(inout):: u3m1aux    (plond,plev)  ! U at previous time step
  real(r8), intent(inout):: v3m1aux    (plond,plev)  ! V at previous time step
  real(r8), intent(out)  :: u3sldaux   (plond,plev)  ! u-wind (time n) (used for advection)
  real(r8), intent(out)  :: v3sldaux   (plond,plev)  ! v-wind (time n) (used for advection)
#endif
...
#ifdef QVORTDAMP
  real(r8) usldmaux   (plond,plev)  ! RHS of U  eqn valid for departure pt (n)
  real(r8) vsldmaux   (plond,plev)  ! RHS of V  eqn valid for departure pt (n)
#endif
...
#ifdef QVORTDAMP
        u3sldaux(i,k) = 2.   *u3aux(i,k) -       u3m1aux(i,k)
        v3sldaux(i,k) = 2.   *v3aux(i,k) -       v3m1aux(i,k)
#endif
...
#ifdef QVORTDAMP
        usldmaux(i,k) = -phisl(i) + v3aux(i,k)*coriol*coslat -href(k,k)*tl(i,k) - bps(k)*dpsl(i)
        vsldmaux(i,k) = -phism(i) - u3aux(i,k)*coriol*coslat -href(k,k)*tm(i,k) - bps(k)*dpsm(i)
#endif
     end do
!
...
#ifdef QVORTDAMP
           usldmaux(i,k) = usldmaux(i,k) - href(l,k)*tl(i,l)
           vsldmaux(i,k) = vsldmaux(i,k) - href(l,k)*tm(i,l)
#endif
        end do
     end do
  end do
...
!
        ...
        v3m1    (i,k) = (v3  (i,k)*coslat + onemeps*vsldm(i,k)*dt)/coslat + &
                                            ...
                                            onemeps*fv   (i,k)*0.5
#ifdef QVORTDAMP
        u3m1aux    (i,k) = (u3aux  (i,k)*coslat + onemeps*usldmaux(i,k)*dt)/coslat + &
                                            onemeps*urhsl(i,k) + &
                                            onemeps*fu  (i,k)*0.5
        v3m1aux    (i,k) = (v3aux(i,k)*coslat + onemeps*vsldmaux(i,k)*dt)/coslat + &
                                            onemeps*vrhsl(i,k) + &
                                            onemeps*fv   (i,k)*0.5
#endif
#ifdef HADVTEST
