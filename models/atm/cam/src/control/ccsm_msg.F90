...
!   interface with cpl6.
!
! CVS Id: $Id: ccsm_msg.F90,v 1.11.2.15 2003/12/12 22:54:57 rosinski Exp $
! 
!-----------------------------------------------------------------------
  ...
  use phys_grid, only: get_ncols_p, nlcols, ngcols, &                   ! Physics parallel decomposition
       ...
       buff_to_chunk, chunk_to_buff, chunks, knuhcs, &
...
!
    if ( .not. allocated(recv2d_chunk) )then
       ...
       if (ierr /= 0) then
          write(6,*)'(CCSMINI) recv2d_chunk allocation error'
          call endrun
       endif
    endif
    allocate(send2d_chunk(pcols,nsnd,begchunk:endchunk), STAT=ierr)
    if (ierr /= 0) then
       write(6,*)'(CCSMINI) send2d_chunk allocation error'
       call endrun
    endif
!
...
!
    if ( .not. allocated(rbuffr) )then
       ...
       if (ierr /= 0) then
          write(6,*)'(CCSMINI) rbuffr allocation error'
          call endrun
       endif
    endif
    allocate(rbuffs(nlcols,nsnd), STAT=ierr)
    if (ierr /= 0) then
       write(6,*)'(CCSMINI) rbuffs allocation error'
       call endrun
    endif
!
...
!
   if (flxave) then
      ...
      if (.not. allocated(recv2d_chunk))then
         allocate(recv2d_chunk(pcols,nrcv,begchunk:endchunk), STAT=ierr)
         if (ierr /= 0) then
            write(6,*)'(READ_RESTART_CCSM) recv2d_chunk allocation error'
            call endrun
         endif
      end if
...
!-----------------------------------------------------------------------
    write(6,*)'(CCSM_MSG_COMPAT): This is revision: $Revision: 1.11.2.15 $'
    write(6,*)'              Tag: $Name:  $'
