...
! !PUBLIC MEMBER FUNCTIONS:
      public mp_init, mp_exit, y_decomp, set_decomp, &
             ...
             mp_scatter4d, mp_gather4d, mp_wrt3d, mp_r8, mp_r4, mp_i4

#if defined(USE_MPI_IO)
      public  mp_file_open, mp_file_write_at, mp_file_close
#endif
!
! Needed for mod_irreg.  Do not use these unless you know what you are doing
!
      public window, r8_win, i4_win, ga_r8_s, ga_r8_r, ga_i4_s, ga_i4_r, &
...
#endif
!------------------------------------------------------------------------------
!  Local variables for use with MPI-2, MPI-1, SHMEM and MLP
!------------------------------------------------------------------------------
...
#define ga_t1_r ga_r8
#define ga_t1_s ga_r8
...
#define t1_win  r8_win
      ...
      real(r4), SAVE, TARGET:: ga_r4a_r(PLON*PLAT*(PLEV+1)*max_nq)
      real(r4), SAVE, TARGET:: ga_r4a_s(PLON*PLAT*(PLEV+1)*max_nq)

      real(r4), SAVE, TARGET:: ga_r4b_r(PLON*PLAT*(PLEV+1)*max_nq)
      real(r4), SAVE, TARGET:: ga_r4b_s(PLON*PLAT*(PLEV+1)*max_nq)
...
!
      ...
      real(r4),    SAVE:: ga_r4_r(PLON*PLAT*(PLEV+1)*max_nq)
      real(r4),    SAVE:: ga_r4_s(PLON*PLAT*(PLEV+1)*max_nq)
...
#if !defined(USE_SHMEM) && !defined(USE_MLP) && !defined(MPI2)
      type (window) :: t1_win
...
#include "mlp_ptr.h"
	...
	bcast_sync = SHMEM_SYNC_VALUE
        ...
        call gotmem
        call forkit
        commglobal = 0   ! Simple communicator for time being
...
!BOC
        ...
        deallocate( zfirst )
        deallocate( zlast )
#if !defined(USE_SHMEM) && !defined(USE_MLP)
#if defined(MPI2)
...
! !LOCAL VARIABLES:
      ...
      numvar    =  3
      isize(1)  =  r8_win%size * max_call * mp_r8
      isize(2)  =  r4_win%size * mp_r4
      isize(3)  =  i4_win%size * mp_i4
...
!BOC
      ...
      ncall_r = ncall_r + 1
#if defined(USE_MLP) || defined(USE_SHMEM)
      if (ncall_r == 1) then
          call mp_barrier()
      endif
#endif
...
! !OUTPUT PARAMETERS:
#if defined(USE_MLP)
      real(r8), intent(inout):: ga_s(win%size*max_call)
      real(r8), intent(inout):: ga_r(win%size*max_call)
#else
...
! !OUTPUT PARAMETERS:
#if defined(USE_MLP)
      real(r4), intent(inout):: ga_s(win%size*max_call)
      real(r4), intent(inout):: ga_r(win%size*max_call)
#else
...
!    02.02.15   Putman
!    02.04.09   Putman	Added ProTeX documentation
!    03.06.06   Sawyer  Added if .. else .. endif statement
!
!EOP
!------------------------------------------------------------------------------
!BOC
...
! !LOCAL VARIABLES:
      ...
      if (win%size >= win%offset_r + win%size_r) then
        ...
        print *, "Fatal ga_recvinit_i4: receive window out of space - exiting"
        call exit(1)
      endif
#endif
!EOC
...
! !OUTPUT PARAMETERS:
#if defined(USE_MLP)
      integer(i4), intent(inout):: ga_s(win%size*max_call)
      integer(i4), intent(inout):: ga_r(win%size*max_call)
#else
