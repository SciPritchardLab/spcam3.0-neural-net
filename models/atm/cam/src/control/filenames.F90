...
!
   ...
   integer, public, parameter :: p_analyses = 100          ! number of analysis files
!
! !PUBLIC DATA MEMBERS:
!
...
!
   ...
   character(len=256), public :: bndtvaer = 'bndtvaer' ! full pathname for time-variant ozone dataset
   character(len=256), public :: aeroptics = 'aeroptics' ! full pathname for time-variant ozone dataset
   character(len=256), public :: co_emis = 'co_emis' ! full pathname for time-variant carbon emission dataset
   character(len=256), public :: dms_emis = 'dms_emis' ! full pathname for time-variant DMS emission dataset
   character(len=256), public :: soil_erod = 'soil_erod' ! full pathname for time-variant soil erodibility dataset
   character(len=256), public :: oxid = 'oxid' ! full pathname for time-variant oxidant dataset
   character(len=256), public :: sox_emis = 'sox_emis' ! full pathname for time-variant SOx emission dataset
   ...
   character(len=256), public :: isccpdata = 'isccpdata' ! full pathname for ISCCP input data
   character(len=256), public :: bndtva(p_analyses) ! full pathname for time-variant analyses of T,U,V,PS,Q

#if defined(QVORTDAMP) || defined(FLUXDAMP) 
   character(len=256), public :: wind3danncycle = 'wind3danncycle'
#endif
#ifdef QRLDAMP
   character(len=256), public :: qrl3danncycle = 'qrl3danncycle'
#endif
...
!   integer, public ::  mss_irt = 365               ! Mass Store retention period for output files
   integer, public :: mss_irt = 0  ! MSP added this to avoid issues with shell commands failing when trying to write things to the NCAR MSS

   integer, public :: m_analyses                   ! current analysis file number
   integer, public :: max_analyses                 ! max number of analysis files
!----------------------------------------------------------------------- 
! EOP
!----------------------------------------------------------------------- 
!
...
!
   ...
   if (home_dir==' ') then
      write(6,*)'INIT_FILEPATHS: Cannot find HOME environment variable'
      call endrun
   end if

   if (nrefrq == 1 .and. len_trim(rest_pfile) == 0 ) then
!      rest_pfile = trim(home_dir) //'/cam2.'// trim(caseid) //'.rpointer'
       rest_pfile = './cam2.'// trim(caseid) //'.rpointer' ! MSP update from home to local dir
       ! (on Kraken Cray, production nodes have no access to home disk)
   end if
!
...
!
   ...
   if ( len_trim(archive_dir) == 0 )then
      ...
      if (logname=='        ') then
         write(6,*)'INIT_FILEPATHS: Cannot find LOGNAME environment variable'
         call endrun
      end if
